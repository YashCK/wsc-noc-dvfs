#!/usr/bin/env python3
"""
Clean plot: Throughput vs Power Cap (two subplots: Control class and Batch class).
...
Throughput source:
  - Uses sweep_results.csv columns: throughput_flits_class0 / throughput_flits_class1
    (as written by scripts/sweep_params.py from throughput.csv).

Selection:
  - For DVFS policies: pick one run per (policy, cap) (default: max throughput for control class).
...
Example:
  python3 scripts/plot_throughput_vs_cap.py \
    ...
    --out plots/throughput_vs_cap.png
"""
...
def _pick_uniform_best_feasible(
    ...
    *,
    ...
    feas = feas.dropna(subset=["throughput_flits_class0", "throughput_flits_class1"])
    if feas.empty:
        return None
    ...
    return feas.loc[feas["throughput_flits_class0"].idxmax()]
...
def _select_points(
    ...
    *,
    ...
    for (pol, cap), g in df.groupby(["policy_norm", "power_cap"], dropna=False):
        ...
        cand = g.dropna(subset=["throughput_flits_class0", "throughput_flits_class1"])
        if cand.empty:
            continue
        if pick_mode == "max_batch":
            rows.append(cand.loc[cand["throughput_flits_class1"].idxmax()].to_dict())
        else:
            rows.append(cand.loc[cand["throughput_flits_class0"].idxmax()].to_dict())
...
def main() -> int:
    ...
    ap.add_argument("--out", default="throughput_vs_cap.png", help="Output image path.")
    ...
    df = df.dropna(subset=["power_cap", "throughput_flits_class0", "throughput_flits_class1"])
    if df.empty:
    ...
    fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(12.2, 4.4), dpi=140, sharex=True)
    ...
    for pol in POLICY_ORDER:
        ...
        style = _style(pol)
        ax0.plot(s["power_cap"], s["throughput_flits_class0"], **style)
        ax1.plot(s["power_cap"], s["throughput_flits_class1"], **style)

    ax0.set_title("Control class throughput (class 0)")
    ax1.set_title("Batch class throughput (class 1)")
    for ax in (ax0, ax1):
